...
def ratios(lines, heights):
    ...
    for i in range(len(lines)):
        if 4855 <= np.round(lines[i]) and np.round(lines[i]) <= 4865:
            hbf = heights[i]
            hb = True
            print('hb', lines[i], heights[i])
        if 5002 <= np.round(lines[i]) and np.round(lines[i]) <= 5012:
            oiiif = heights[i]
            oiii = True    
            print('oiii', lines[i], heights[i])
        if 6543 <= np.round(lines[i]) and np.round(lines[i]) <= 6553:
            nii1f =heights[i]
            nii1 = True
            print('nii1', lines[i], heights[i])
        if 6557 <= np.round(lines[i]) and np.round(lines[i]) <= 6567:
            haf = heights[i]
            ha = True
            print('ha', lines[i], heights[i])
        if 6578 <= np.round(lines[i]) and np.round(lines[i]) <= 6588:
            nii2f = heights[i]
            nii2 = True
            print('nii2',lines[i], heights[i])
        if 6711 <= np.round(lines[i]) and np.round(lines[i]) <= 6721:
            sii1f = heights[i]
            sii1 = True
            print('sii1', lines[i], heights[i])
        if 6726 <= np.round(lines[i]) and np.round(lines[i]) <= 6736:
            sii2f = heights[i]
            sii2 = True
            print('sii2',lines[i], heights[i])
...
def plotpeaks(snr, sn_threshold, prom, w, ax, real, galnorm, linenames, linelist):
    
    line_peaks, line_properties = find_peaks(snr, height=sn_threshold, prominence=prom,width=w) 
